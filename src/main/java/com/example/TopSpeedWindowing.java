    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Tuple4<Integer, Integer, Double, Long>> topSpeeds =
                carData.assignTimestampsAndWatermarks(new CarTimestamp())
                        ...
                        // 10s 内行驶距离超过50m的最高车速,清除10s 外的数据
    ...
    private static class CarSource
        ...
        public void run(SourceContext<Tuple4<Integer, Integer, Double, Long>> ctx)
            ...
            while (isRunning) {
                Thread.sleep(1000);
                if (rand.nextBoolean()) {
                    speeds[0] = Math.min(100, speeds[0] + 5);
                } else {
                    speeds[0] = Math.max(0, speeds[0] - 5);
                }
                Tuple4<Integer, Integer, Double, Long> record = null;
                long now = System.currentTimeMillis();
                long i = (now - time)/1000;
                if(i ==1){
                    record =
                            new Tuple4<>(
                                    0,
                ...
                System.out.println("input-data:"+record);
                ctx.collect(record);
